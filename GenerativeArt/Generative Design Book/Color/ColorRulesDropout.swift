    ...
    func generateColors() {
        ...
        for i in 0..<colorCounts[colorCount] {
            if i.isMultiple(of: 2) {
                ...
                saturationValues.append(Double.random(in: 0...0.5))
                ...
                Canvas { context, size in
                    ...
                    for row in 0..<rows {
                        ...
                        for i in 0..<parts.count {
                            ...
                            let complement = Color(hue: 1-hue, saturation: sat, brightness: bri)
                            colorIdx += 1
                            
                            
                            let gradientCenter = CGPoint(x: rect.midX, y: rect.midY)
                            let endRadius = max(width, height)
                            
                            context.fill(Path(rect), with: .radialGradient(Gradient(colors: [color, complement]), center: gradientCenter, startRadius: 0, endRadius: endRadius))
                        }
                    }
                }
...
struct ColorRulesDropout_Previews: PreviewProvider {
